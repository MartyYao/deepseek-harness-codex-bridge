...
> 与市面上其他「使用 GPT 订阅/会员」的插件不同：**本插件不需要 API Key、不接触登录 Cookie、也不通过任何第三方访问你的订阅**——认证完全由你本机官方 Codex CLI 自己持有，请求直达官方通道。这是本插件的核心优势：

- **零凭据**：插件不读取、不保存、不传输任何 ChatGPT 账号凭据。认证由你**本机已登录的官方 Codex CLI**（`codex login`，官方 OAuth 设备码流程）自己持有。
