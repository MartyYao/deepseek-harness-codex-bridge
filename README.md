...
> **本插件没有任何账号风险，与市面上其他「使用 GPT 订阅/会员」的插件有本质区别。**

常见的 GPT 订阅类插件需要你把 **API Key、登录 Cookie 或账号凭据**交给插件进程或第三方服务器——凭据一旦外泄，等于把你的订阅账号交给别人。本插件**完全不同**：

- **零凭据接触**：插件不读取、不保存、不传输任何 ChatGPT 账号凭据。认证完全由你**本机已登录的官方 Codex CLI**（`codex login`，走官方 OAuth 设备码流程）自己持有。
- **零外部请求**：插件只连接**本机回环地址**（`ws://127.0.0.1:4500`）上的 codex app-server，不发起任何第三方网络请求；你的流量走 codex CLI → chatgpt.com 官方通道，插件全程只是本机协议翻译。
- **无账号共享/池化**：不使用、不接入任何共享账号或账号池，每次执行都使用你本人已登录的官方身份。
- **可审计**：全仓库无任何 token/凭据处理代码，`npm test` 与 `scripts/verify-integration.mjs` 验证的也是协议行为，不含认证材料。

换言之：**这个插件只是「在 dsh 里帮你按一下官方 Codex 的按钮」，按钮本身是你自己的、凭据也一直在你手里。**
